...
class AccessPatternAnalyzer {
  ...
  recordAccess(key) {
    ...
    if (this.accessHistory.length >= 2) {
      ...
      const sequences = this.sequenceMap.get(prevKey) || /* @__PURE__ */ new Map();
  ...
  predictNext(currentKey, topN = 5) {
    ...
    const sorted = Array.from(sequences.entries()).sort((a, b) => b[1] - a[1]).slice(0, topN).map(([key]) => key);
   ...
   * 获取最频繁访问的键
   */
  getMostFrequent(topN, exclude = /* @__PURE__ */ new Set()) {
    return Array.from(this.accessCount.entries()).filter(([key]) => !exclude.has(key)).sort((a, b) => b[1] - a[1]).slice(0, topN).map(([key]) => key);
  }
  /**
   * 获取访问模式统计
   */
  getStats() {
    ...
    const hotKeys = Array.from(this.accessCount.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([key, count]) => ({
      key,
      count
    }));
    const patterns = [];
...
class LRUPrefetchStrategy {
  ...
  predict(currentKey, _history) {
    return this.recentKeys.filter((key) => key !== currentKey).slice(0, 5);
  }
  updateHistory(key) {
...
class AssociationRuleStrategy {
  ...
  predict(currentKey, _history) {
    ...
    return Array.from(associated).sort((a, b) => (coMap.get(b) || 0) - (coMap.get(a) || 0)).slice(0, 5);
  }
  updateHistory(_key) {
  }
  /**
  ...
  addRule(key, associatedKeys) {
    const set = this.rules.get(key) || /* @__PURE__ */ new Set();
    associatedKeys.forEach((k) => set.add(k));
    this.rules.set(key, set);
    const coMap = this.coOccurrence.get(key) || /* @__PURE__ */ new Map();
...
function createPrefetchManager(config) {
  return new PrefetchManager(config);
}

export { PrefetchManager, createPrefetchManager };
