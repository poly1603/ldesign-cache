...
export class ErrorHandler {
    ...
    static async safeAsync(operation, options = {}) {
        const { logError = true, logPrefix = '[ErrorHandler]', errorTransform = err => (err instanceof Error ? err : new Error(String(err))), } = options;
        try {
    ...
    static safeSync(operation, options = {}) {
        const { logError = true, logPrefix = '[ErrorHandler]', defaultValue, rethrow = false, errorTransform = err => (err instanceof Error ? err : new Error(String(err))), } = options;
        try {
            return operation();
        }
        catch (error) {
     ...
     * @returns 执行结果
     */
    static async withRetry(operation, maxRetries = 3, delay = 1000, options = {}) {
