...
import { CACHE_MANAGER_KEY, CacheProvider, } from '../vue/cache-provider';
/**
 * 默认配置
 */
const defaultConfig = {
 ...
 * ```
 */
export function createCacheEnginePlugin(options = {}) {
    // 合并配置
    const config = { ...defaultConfig, ...options };
    const { name = 'cache', version = '1.0.0', dependencies: _dependencies = [], autoInstall = true, enablePerformanceMonitoring = false, debug = false, } = config;
    if (debug) {
