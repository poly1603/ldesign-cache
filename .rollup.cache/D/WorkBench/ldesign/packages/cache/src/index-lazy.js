...
export const lazyModules = {
    ...
    // 存储引擎 - 按需加载
    async loadEngines() {
        const [{ MemoryEngine }, { LocalStorageEngine }, { SessionStorageEngine }, { IndexedDBEngine }, { CookieEngine },] = await Promise.all([
    ...
    // 安全模块 - 按需加载
    async loadSecurity() {
        const [{ SecurityManager }, { AESCrypto }, { KeyObfuscator },] = await Promise.all([
    ...
    async loadStrategies() {
        const [{ StorageStrategy }, evictionStrategies,] = await Promise.all([
    ...
    // Vue 集成 - 按需加载
    async loadVue() {
        const [{ useCache }, { useCacheStats }, vueHelpers, { CacheProvider },] = await Promise.all([
 ...
 * 只加载核心功能，其他功能按需加载
 */
export async function createLazyCacheManager(options) {
...
export async function loadAdvancedModules() {
    const [performanceMonitor, syncManager, warmupManager, cacheAnalyzer,] = await Promise.all([
...
export async function loadAllModules() {
    const [engines, security, strategies, utils, vue, presets, advanced,] = await Promise.all([
        lazyModules.loadEngines(),
