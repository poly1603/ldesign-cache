...
class IndexedDBEngine extends baseEngine.BaseStorageEngine {
  constructor(config) {
    super();
  ...
  get available() {
    return typeof window !== "undefined" && "indexedDB" in window && window.indexedDB !== null && this.db !== null;
  }
  /**
   * 初始化数据库
   */
  async initialize() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        this.updateUsedSize().then(() => resolve()).catch(console.error);
      };
    ...
    try {
      ...
      if (cursor) {
        ...
        cursor.onsuccess = (event) => {
          ...
          if (cursor2) {
            deletePromises.push(this.executeRequest(store.delete(cursor2.primaryKey)));
            cursor2.continue();
          }
        };
      }
...
//# sourceMappingURL=indexeddb-engine.cjs.map
