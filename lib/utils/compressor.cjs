...
class Compressor {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      minSize: options.minSize ?? performance.DATA_SIZE.COMPRESSION_MIN_SIZE,
      level: options.level ?? performance.COMPRESSION.DEFAULT_LEVEL,
  ...
  arrayBufferToBase64(buffer) {
    ...
    const chunkSize = performance.DATA_SIZE.COMPRESSION_CHUNK_SIZE;
    const chunks = [];
  ...
  isCompressed(data) {
    ...
    return entropy > performance.DATA_SIZE.COMPRESSION_ENTROPY_THRESHOLD;
  }
  /**
   * 计算数据熵值
   */
  ...
  getCompressionStats(data) {
    ...
    if (originalSize < this.options.minSize) {
      ...
      recommendedAlgorithm = "none";
    } else if (originalSize < performance.PERFORMANCE_THRESHOLDS.SMALL_FILE_SIZE) {
      recommendedAlgorithm = "deflate";
    } else {
      recommendedAlgorithm = "gzip";
    }
    const estimatedRatio = recommendedAlgorithm === "none" ? performance.COMPRESSION.NO_COMPRESSION_RATIO : performance.COMPRESSION.TYPICAL_RATIO;
...
//# sourceMappingURL=compressor.cjs.map
