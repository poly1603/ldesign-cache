...
class MemoryManager {
  constructor(config = {}) {
    ...
    this.config = {
      maxMemory: config.maxMemory || performance.MEMORY_CONFIG.MAX_DEFAULT,
      highPressureThreshold: config.highPressureThreshold || performance.MEMORY_CONFIG.HIGH_PRESSURE_THRESHOLD,
      criticalPressureThreshold: config.criticalPressureThreshold || performance.MEMORY_CONFIG.CRITICAL_PRESSURE_THRESHOLD,
      autoCleanupInterval: config.autoCleanupInterval || performance.TIME_INTERVALS.CLEANUP_INTERVAL_DEFAULT,
  ...
  getPressureLevel(usagePercentage) {
    ...
    if (usagePercentage >= performance.MEMORY_CONFIG.MEDIUM_PRESSURE_THRESHOLD) {
      return "medium";
    }
    return "low";
  }
  ...
  checkMemoryPressure() {
    ...
    } else if (stats.pressureLevel === "high" && Date.now() - this.lastCleanupTime > performance.TIME_INTERVALS.HIGH_PRESSURE_CLEANUP_INTERVAL) {
      this.performCleanup();
    }
  }
  /**
  ...
  performCleanup() {
    ...
    this.cleanupCount++;
    if (this.metadataPool.size > performance.MEMORY_CONFIG.POOL_CLEANUP_THRESHOLD) {
      this.metadataPool.clear();
    }
    if (this.cacheItemPool.size > performance.MEMORY_CONFIG.POOL_CLEANUP_THRESHOLD) {
  ...
  calculateObjectSize(obj) {
    if (obj === null || obj === void 0) return performance.DATA_SIZE.NULL_SIZE;
    if (typeof obj === "boolean") return performance.DATA_SIZE.BOOLEAN_SIZE;
    if (typeof obj === "number") return performance.DATA_SIZE.NUMBER_SIZE;
    ...
    if (typeof obj === "object") {
      ...
      if (Array.isArray(obj)) {
        size = performance.DATA_SIZE.ARRAY_OVERHEAD;
        for (const item of obj) {
          size += this.calculateObjectSize(item);
        }
      } else {
        size = performance.DATA_SIZE.OBJECT_OVERHEAD;
        for (const key in obj) {
    ...
    return performance.DATA_SIZE.NUMBER_SIZE;
  }
  /**
   * 计算字符串大小（UTF-8）优化版本
   */
  calculateStringSize(str) {
    let size = performance.DATA_SIZE.STRING_OVERHEAD;
...
//# sourceMappingURL=memory-manager.cjs.map
