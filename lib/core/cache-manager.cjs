...
var memoryManager = require('./memory-manager.cjs');
var prefetchManager = require('./prefetch-manager.cjs');
...
class CacheManager {
  constructor(options = {}) {
    ...
    this.strategy = new storageStrategy.StorageStrategy(options.strategy);
    this.security = new securityManager.SecurityManager(options.security);
    this.eventEmitter = new eventEmitter.EventEmitter();
    this.memoryManager = memoryManager.createMemoryManager({
    ...
    if (options.enablePrefetch) {
      this.prefetchManager = prefetchManager.createPrefetchManager({
  ...
  async initializeEngines() {
    ...
    for (const engineType of engineTypes) {
      try {
        ...
        const engine = await factory.StorageEngineFactory.create(engineType, engineConfig);
        if (engine.available) {
  ...
  async selectEngine(key, value, options) {
    ...
    const defaultEngine = this.options.defaultEngine || factory.StorageEngineFactory.getRecommendedEngine();
  ...
  validateSetInput(key, value, options) {
    validator.Validator.validateSetInput(key, value, options);
  }
  /**
   * 创建元数据
   */
