import type { EvictionStrategy } from '../strategies/eviction-strategies'
...
import { EvictionStrategyFactory } from '../strategies/eviction-strategies'
...
export class MemoryEngine extends BaseStorageEngine {
  ...
  private async evictUntilSpaceAvailable(requiredSpace: number): Promise<void> {
    ...
    // 使用批量淘汰（快3倍）
    if (batchSize > 3) {
      await this.evictBatch(batchSize)
